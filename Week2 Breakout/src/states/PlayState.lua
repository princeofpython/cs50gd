...
function PlayState:enter(params)
    ...
    self.power = Powerup(2)
    self.key = Powerup(10)

    self.lock = params.lock  -- locked block is on screen
...
function PlayState:update(dt)
    ...
    if timer > randtime then
        timer = 0
        randtime = math.random( 4, 7 )
        if alternate then
            self.power.onScreen = true
        elseif self.paddle.locked and self.lock then
            self.key.onScreen = true
        end
    ...
    if self.power:collides(self.paddle) then
        ...
        self.balls[#self.balls]:newReset()
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                ...
                if brick.locked then
                    if not self.paddle.locked then
                ...
                if brick.locked then
                    if not self.paddle.locked then
                        brick:hit()
                        self.lock = false -- locked block is no more on screen
                    else
                        gSounds['block']:play()
                    end
                else
    ...
    for i, ball in pairs(self.balls) do
        if ball.y >= VIRTUAL_HEIGHT then
            ball.remove = true
            if #self.balls == 1 then -- if it is the last ball, reduce a life
