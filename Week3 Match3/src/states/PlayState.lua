    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                swapTiles(self.highlightedTile, newTile, self.board.tiles)
                ...
                :finish(function()
                    if self.board:calculateMatches() then
                        ...
                        -- if it is not a valid board, initialize the tiles again
                        --if not self.board:validBoard() then
...
function swapTiles(highlightedTile, newTile, tiles)
    ...
    tiles[highlightedTile.gridY][highlightedTile.gridX] = highlightedTile
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for j, tile in pairs(match) do
                if tile.shiny then
                    if j < #match then
                        if match[j+1].gridX == tile.gridX then
                            self.score = self.score + 7 * 50
                            self.timer = self.timer + 7
                        else
                            self.score = self.score + (8 - #match)* 50
                            self.timer = self.timer + (8 - #match)
                        end
                    else
                        if match[j-1].gridX == tile.gridX then
                            self.score = self.score + 7 * 50
