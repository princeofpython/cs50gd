...
function AlienLaunchMarker:update(dt)
    ...
    if self.launched and (not self.alien.hit) then
        if love.keyboard.wasPressed('space') then
            ...
            self.alien1 = Alien(self.world, 'round', x, y, 'Player')
            self.alien1.body:setLinearVelocity(Vx, Vy + 40)

            self.alien2 = Alien(self.world, 'round', x, y, 'Player')
            self.alien2.body:setLinearVelocity(Vx, Vy - 40)
