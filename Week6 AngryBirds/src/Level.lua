...
function Level:update(dt)
    ...
    if self.playerCollided then
        self.launchMarker.alien.hit = true
    end
    ...
    if self.launchMarker.launched then
        ...
        if not self.launchMarker.tripled then
            ...
            local xPos, yPos = self.launchMarker.alien.body:getPosition()
            local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()
            local xPos1, yPos1 = self.launchMarker.alien1.body:getPosition()
            ...
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                if xPos1 < 0 or (math.abs(xVel1) + math.abs(yVel1) < 1.5) then
