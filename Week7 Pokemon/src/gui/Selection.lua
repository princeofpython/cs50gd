...
function Selection:update(dt)
    if self.needCursor and love.keyboard.wasPressed('up') then
        ...
        gSounds['blip']:stop()
        gSounds['blip']:play()
    elseif self.needCursor and love.keyboard.wasPressed('down') then
        ...
        gSounds['blip']:stop()
        gSounds['blip']:play()
    elseif self.needCursor and love.keyboard.wasPressed('return') or love.keyboard.wasPressed('enter') then
...
function Selection:render()
    ...
    for i = 1, #self.items do
        ...
        if self.needCursor and i == self.currentSelection then
